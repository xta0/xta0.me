...
在汽车的实际行驶中，它出现的位置是和是和时间相关的函数，即位置的概率分布是连续的，而不是上面demo中的离散概率分布。但定位的原理是不变的，即不断的sense和move。对于连续型的概率分布，我们可以使用更好的工具来计算，这个工具就是卡尔曼滤波器。

> 对于卡尔曼滤波器，网上可以找到很多资料

回顾前面介绍Localization的步骤，第一步是需要有一个先验概率，一个连续型的概率密度函数，对于卡尔曼滤波来说，这个函数可以使用高斯函数。
...
这一节我们将用Python定义一个类型为`Car`的object，它具有以下功能：
