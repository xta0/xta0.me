...
如上图所示，有两个等价类集合$S_1,S_2$。在$S_1$中，有$<3,5>,<5,7>,<3,7>$ 三个等价对，由等价关系可知3,5,7彼此等价，因此它们具有相同的根节点，用1表示；$S_2$同理，假如现在引入另一个等价对$<3,2>$，可以发现，3的根节点为2，不等于1，因此如果要建立等价关系，需要将$S_1,S_2$合并，这样两个集合中的元素都将有相同的根节点。
