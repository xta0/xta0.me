---
list_title:   Deep Learning | Shallow Neural Networks
...
- $z^{[1]}$是`4x1`，$W^{[1]}$是`4x3`，$x$是`3x1`，$b^{[1]}$是`4x1`，$a^{[1]}$是`4x1`
- $z^{[2]}$是`1x1`，$W^{[2]}$是`1x4`，$a^{[2]}$是`1x1`，$b^{[2]}$是`1x1`
...
如果神经网路的某个 Layer 要求输出结果在`[0,1]`之间，那么选取$\sigma(x) = \frac{1}{1+e^{-x}}$作为 Activation 函数，此外，则可以使用**Rectified Linear Unit**函数：
