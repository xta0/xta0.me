...
这种情况下是不能读写都加锁的，由于写操作频率很低，导致则大量的读操会被无意义的锁block，性能会大大降低；如果只给写操作加锁，读操作不加锁，则会发生`race condition`的情况，比如有可能读到写了一半的不完整数据。因此我们需要一种新的锁结构来解决上面问题，我们希望这总锁可以让读线程并发访问不受影响，只当写操作时block读线程，等写操作完成后，读线程继续执行。这种锁叫做“读写锁”，它用来处理写操作不频繁，读操作频繁的场景。所有的读写锁都需要遵循下面三个原则：

1. 允许多个线程同时读取共享变量
2. 只允许一个线程写共享变量
3. 如果一个线程正在执行写操作，此时禁止线程读取变量

读写锁与互斥锁的一个重要区别就是读写锁允许多个线程同时读取变量，而互斥锁是不允许的，这是读写锁在读多写少场景下性能由于互斥锁的关键。但是**读写锁本身的操作时互斥的**，但一个线程在写时，其它线程是不允许进行读写操作的。C++ 17标准库新增了`shared_mutex`对应这类读写锁的实现。
...
```
在上面的场景中，如果某个线程想进行写操作，则需要等所有读操作的线程执行完，并释放`shared_mutex`之后才能执行。同理，当写线程执行时，读操作线程被block，无法获取`shared_mutex`，写线程也需要等待当前的写线程完成后，才能重新获取`shared_mutex`。
