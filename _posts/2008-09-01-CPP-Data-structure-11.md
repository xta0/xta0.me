---
layout: post
list_title: Data Structre Part 10 | Indexing | 索引
title: 搜索(二)：索引 | Indexing
# sub_title: Searching
mathjax: true
---
...
平时我们在搜索引擎中输入关键字进行检索，会首先根据对关键字的索引得到一组倒排列表作为结果，然后对该结果进行一些算法调整（广告）最终得到一组网页序列返回。所谓对文本文件的倒排是指以文本文件中出现的词（word）作为一个索引项，得到它一个倒排列表，这个列表包含了该词出现在哪一篇文章（网页），以及文章中的位置，或者哪一个文章里面出现了多少次等等的信息。
...
基于词的索引是使用最广泛的建索引的方式，它需要一个已经排过序的关键词的列表,其中每个关键词和一个指针组成一个索引对，该指针可以指向该关键词出现文档集合或者这个单词在文档中的位置，举例如下，假设我们有6个文档，每个文档都是长字符串：
...
全文索引的基本思想是，把正文看作一个长的字符串在数据结构中记录的是<mark>子字符串的开始位置</mark>，查询就可以针对正文中的任何子字符串。建立正文倒排文件，需要：
...
3.  建立正文倒排表、倒排文件得到各个关键词的集合对于每一个关键词得到其倒排表，然后把所有的倒排表存入文件
...
1. 在倒排文件中检索关键词
2. 第二步，如果找到了关键词，那么获取文件中的对应的倒排表，并获取倒排表中的记录。
