...
T1 sum(T2 x1, T3 x2);
```
上述代码中，编译器可以根据出`x1`和`x2`来推测出`T1`和`T2`的类型，但是没法推测出`T3`的类型，因此需要调用者手动指定

```cpp
auto val = sum<long long>(i, lng); //long long sum(int, long)
```
此时，我们告诉编译器`T1`是`long long`，而`T2`和`T3`编译器可自行推断出。这里需要注意的是，显示指定的参数类型是从左到右依次展开，即如果模板定义为
...
```
此时`fn`函数的返回值类型可由迭代器类型`It`推到出来，因此我们不必手动指定其类型。

如果我们继续追问让上述函数返回一个“值”而非引用时，`decltype(*beg)`就不奏效了，因为迭代器永远返回引用类型。此时我们需要用`type_traits`头文件中的函数。
