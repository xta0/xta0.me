...
上面可以看到第一层Hidden Layer的参数，$\theta_{ij}$表示从节点`i`到节点`j`的权重值，$\theta^{(1)}$表示该权重位于第一层。如上图中的$\theta$矩阵是`3x4`的，输入的feature矩阵是`4x1`的，这样相乘得出的第一层输出矩阵是`3x1`的，对应每个`a`节点的值。而整个神经网络最终输出结果是神经元`a`矩阵再乘以第二层Hidden Layer的参数矩阵$\theta^{(2)}$。由此我们可以推测出$\theta$矩阵的维度的计算方式为:
