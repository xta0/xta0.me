---
list_title:  MIT 6.172 (4) - Assembly Language and Computer Architecture
...
The instruction set architecture (ISA) specifies the syntax and semantics of assembly, which includes Registers, Instructions, Data types and Memory addressing modes.
...
- The Flags register keeps track of whether there is an overflow. Or whether the last operation returns 0, etc.
...
Here are all the general purpose registers
...
- An opcode for data movement, arithmetic or logic uses a single-char suffix to indicate the data type
...
Conditional jumps and conditional moves use a one or two char suffix to indicate the condition code.
...
In the example above, `ne` means the jump should only be taken if the arguments of the previous comparison are not equal. The previous comparison result is stored in the RFLAG register. 
...
A: A couple of hundreds. So clearly, if you can get things into registers, most registers you can access in a single cycle.
...
- The AVX-512 instructions increase the register length to 512 bits and provide new vector operations, including popcount.
