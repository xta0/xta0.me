...
要理解贝叶斯定理，先要复习一下什么是条件概率和全概率。所谓条件概率是指在某件事发生的前提下，另一件事发生的概率。和独立概率比起来条件概率不是很好理解，因为平时生活中能直接使用上这种概率的机会比较少，一个经典条件概率实验是红蓝球实验:
...
根据文氏图，可以很直观地看到在事件A发生的情况下事件B发生的概率是交集部分的概率，因此，对于条件概率也可以这么理解，我们假设事件A有N个样本，某一次事件A发生了，其中有M个样本恰好是B的样本，则条件概率为`M/N`，它可以认为是在<mark>在A的样本中寻找B的份额占比</mark>。
...
左图中，`A`和`A'`构成样本空间`S`，即 `P(A)+P(A')=1`。在这种情况下，右图中`B`发生的概率为 `P(B) = P(B∩A) + p(B∩A')`，有前面的推导可知将 `P(B∩A)=P(B|A)P(A)`，带入化简，得到全概率公式:

$$
P(B) \thinspace = \thinspace P(B|A)P(A) + P(B|A')P(A')
$$

它的含义是，如果`A`和`A'`构成样本空间的一个划分，那么事件`B`的概率，就等于`A`和`A'`的概率分别乘以B对这两个事件的条件概率之和。为了加深对全概率公式的理解，我们同样给出一个例子，这个例子是说
...
<div><img src="{{site.baseurl}}/assets/images/2018/07/ad-3.png"></div>
<div><img src="{{site.baseurl}}/assets/images/2018/07/ad-4.png"></div>
</div>
