...
- 文件操作步骤
	...
	//使用构造函数打开文件
	ofstream fout("clients.dat", ios::out | ios::binary); //以二进制形式打开文件，修改文件会删除原内容
	...
	- 读写文件，使用文件指针
		...
		```cpp
		//打开文件，移动写指针
		ofstream fout("a1.out",ios::app); //写文件，对文件的修改追加到文件尾部
		long location = fout.tellp();
		location = 10L; //写指针移动到第10个字节处
		fout.seekp(location); //将写指针移到location处
		fout.seekp(location,ios::beg); //从文件头开始，向后移动location个字节
		fout.seekp(location,ios::cur); //从当前文件指针开始，向后移动location个字节
		fout.seekp(location,ios::end); //从文件尾部开始，向前移动location个字节，这时location为负数
		//写文件
		int x = 10;
		fout.seekp(20,ios:beg);
		fout.write((const char* )&x, sizeof(int)); //将x写入
		//关闭文件
		fout.close();
		```
		- 读文件
		...
		```cpp
		//打开文件，移动读指针
		ifsteam fin("a1.in",ios::in) //读文件
		long location = fin.tellg();
		location = 10L; //读指针移动到第10个字节处
		fin.seekg(location); //将读指针移到location处
		fin.seekg(location,ios::beg); //从文件头开始，向后移动location个字节
		fin.seekg(location,ios::cur); //从当前文件指针开始，向后移动location个字节
		fin.seekg(location,ios::end); //从文件尾部开始，向前移动location个字节，这时location为负数
		//读文件
		int x;
		fin.seekg(20,ios:beg);
		fin.read((char* )&x, sizeof(int)); //将x读入
		//关闭文件
		fin.close();
		```
	- 关闭文件
