...
public:
    ...
    virtual void vf2() { cout<<__func__<<endl; }
...
![](/assets/2009/09/vtbl.png)

可以把虚表理解为一个指针数组，数组中的每个元素是一个指针变量，指向某个函数地址。运行时如果发生动态绑定则通过`(*(p->vptr)[n])(p)`进行函数寻址

使用多态可以让某个基类具有类似“接口”的能力，对于某些场景，我们只需要传入具有虚函数的基类对象即可，也就是所谓的面向接口编程。
