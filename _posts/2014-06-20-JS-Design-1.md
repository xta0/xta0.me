...
上述代码中，`logName`是一个独立的函数，由前面的讨论可知，此时`this`指向global object，也就是`window`,由于`windown`中没有`getFullName`这个方法，因此上述代码执行会出错。针对上面的情况，在JavaScript中，`this`的值是可以改变的，我们可以让`this`指向一个有`getFullName`方法的对象：
