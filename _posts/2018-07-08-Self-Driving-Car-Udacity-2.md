...
...
```
简单总结一下，机器人通过传感器不断引入观测数据，从而完成了将原先均匀的概率分布提升为包含一定位置的概率分布。sense的本质上是一种<mark>引入信息的过程</mark>
...
接下来我们让我回到卡尔曼滤波过程，根据前面的定位原理可知，在有了先验概率后，就可以开始sense了，根据贝叶斯定理，新的高斯函数应该等于先验高斯函数乘以调整因子，两个高斯函数乘积的性质在上面一节中已经讨论过，因此我们可以直接写出sense的代码：
