...
int *p2 = &value++; //错误
```
这个例子中，`p1`绑定的是一个左值，因此可以进行取地址操作，`++value`实际上就是对`value`自身的值加1，而对于`p2`，编译器给出的错误是`error: cannot take the address of an rvalue of type 'int'`，这说明此时`value`为右值，实际上，`p2`绑定的并不是`value`而是一个临时变量，我们可以将上述代码展开，它等价于
...
value = value +1;
```
这里为了理解方便，增加了一个`tmp`的变量，而实际编译过程中，1，2步是合在一起的，因此不存在`tmp`这个左值，只有一个临时的右值，`p2`绑定的是一个临时的右值，因此编译报错。
...
Dummy &ld = func(); //错误
const Dummy &ld = func(); //const左值绑定右值
const Dummy &ld = Dummy(); //
```
