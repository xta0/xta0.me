...
The idea of exploiting **sparsity** is to avoid storing and computing on zeros. "The fastest way to compute is not to compute at all".
...
We can inline the `square` function to reduce the overhead (stack expense). To do that, we can simply mark the function using `static inline` keywords. It turns out for some modern compilers, the functions can be inlined without declaring "static inline".

Inline functions can be just as efficient as macros, and they are better structured.
