...
本节我们来讨论下一个简单的C程序的编译，我们将把重点放在目标文件的链接于加载上。
...
```
为了生成目标文件`.o`，我们可以使用gcc来进行编译，我们先来要编译`function.c`，得到`function.o`: `gcc -c function.c -o function.o`。该文件包含了很多重要的信息

- 可以将`.o`理解为所有symbol的集合，可以用`nm`命令查看其包含的symbol
