---
updated: "2019-03-10"
list_title: Linode快速上手（一） | Setup HTTPs and Nginx on Linode Server  
...
[Certbot](https://certbot.eff.org/)是一个HTTPs证书管理工具，和Let's Encrypt搭配使用非常方便。参考官方文档，执行下面命令
...
```
该命令会在每周三凌晨2点半检查证书是否过期，如果过期则自动更新，成功后执行`--post-hook`后的命令，重启nginx。默认情况下Let's encrpyt会log日志，路径为`/var/log/letsencrypt`，我们也可以重定向日志到自定义文件中，方便查阅。
