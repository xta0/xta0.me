...
接下来我们来一起分析一些动态规划问题的经典例子，举些例子的目的是为了观察DP问题的形式以及解DP问题的一般规律。DP之所以难是因为它的解法通常不直观，子问题不容易想到，进而找到不到正确的状态转移方程。想要熟练的解DP问题没有什么特别好的办法，只有多练，然后总结，然后再练，如此多循环几次就多少会找到一些感觉。结合其它一些参考资料，我这里总结了解DP问题几条思维方法：

1. 递归+记忆化 = 递推，如果能用记忆化递归实现的算法，也可以转化为使用递推公式进行求解
2. 状态的定义与表示，使用一维数组或是多维数组
3. 状态转移方程，最难的部分，推导状态之间的关系
4. 最优子结构，全局最优问题能否转化为局部最优问题
