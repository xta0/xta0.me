...
2. 尝试计算`L[i]`
    ...
    如何才能得到`a[i]`位置的最优LIS呢？我们先来思考一下为什么前面会漏掉`[-3,1,4,5]`这个最优解。不难想到，原先的逻辑是拿`a[i]`和LIS序列的末尾进行比较，如果`a[i]`大，则追加到LIS序列后面，但是如果`a[i]`小呢？上面的推导并没有考虑这种情况，实际上当`a[i]`小于LIS末尾元素时，仍有可能产生一个以`a[i]`结尾的新的LIS。该LIS要优于原LIS。实际上我们上面所用的策略是贪心策略，感觉上我们每一步都是最优解，但实际得到的并不是全局最优解。
