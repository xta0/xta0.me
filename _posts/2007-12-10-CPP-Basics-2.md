...
<mark>C++ 11</mark>中引入了三类智能指针，定义在`<memory>`头文件
...
|--|--|
|`make_shared<T>(args)` | 返回一个`shared_ptr`|
|`shared_ptr<T> p (q)` | `p`是`q`的拷贝，`q`引用计数+1|
|`p=q` | p的引用计数-1，`q`引用计数+1|
|`p.unique()` | 若`p.use_count()`为1，返回true，否则返回false|
|`p.use_count()` | 返回`p`的引用计数,速度慢，用于调试 |

- 创建

最安全的方式是使用`make_shared<T>`

```cpp
//创建指向
shared_ptr<string> s = make_shared<string>(1,'c');
